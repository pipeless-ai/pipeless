...
def hook(frame, context):
    rgb_frame = frame['modified']
    prediction = next(context['model'](rgb_frame, stream=True))
    bounding_boxes = prediction.boxes.data.tolist() if prediction.boxes else []
    for box in bounding_boxes:
        x1, y1, x2, y2, score, class_number = box
        box_label(rgb_frame, [x1, y1, x2, y2], yolo_classes[int(class_number)], score, (255, 0, 255))

    frame['modified'] = rgb_frame
